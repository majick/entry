...
const request = mock((number: number) =>
    fetch("http://localhost:8080/api/new", {
        ...
        body: [
            `CustomURL=encrypted-test-${number}`,
            `Content=${number}`,
            `EditPassword=encrypted-test-${number}`,
            `ViewPassword=encrypted-test-${number}`,
...
// create test
describe("app-usage", () => {
    test("create-paste-encrypted", async () => {
        const start = performance.now();
        const beforeMem = process.memoryUsage.rss();

        // run test
        console.log("Sending 100 create-paste-encrypted requests to /v");
